...
Slack通知は `capture_lead` に加えて、2026-07-06時点では `SLACK_NOTIFY_PARTIAL_LEADS=true` をCloudflare Secretへ設定し、`partial_lead` も問い合わせチャンネルへ流す。本番で通知量が多すぎる場合は、このSecretを外すか `false` に戻せば、partialはNotion保存だけにできる。

通常のお問い合わせフォームも同じWorkerの `contact_form` action経由でSlack通知する。既存のGoogle Apps Script送信は維持し、Slack通知は並列で実行する。
