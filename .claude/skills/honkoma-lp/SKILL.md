...
9. **whileInView(Reveal系)の目視確認は実ブラウザ(claude-in-chrome)で行う。** プレビューMCPは隠れタブで IntersectionObserver が発火せず「動いてない」と誤判定する。
...
2. `npm run build` が通る
3. preview で描画・崩れなし
4. **Reveal等のスクロール演出は claude-in-chrome の実ブラウザで発火確認**
5. スクショで proof
